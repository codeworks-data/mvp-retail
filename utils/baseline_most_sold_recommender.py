...
class MostSoldRecommender:
    def fit(self, cumulative_sales: pd.DataFrame) -> None:
        """
        Set as attributes the most sold items for known customers, and overall most sold items for unknown customers
    ...
    def predict(self, users_ids: List) -> pd.DataFrame:
        """
        Return the ordered (by quantity) list of items sold for each user
        :param users_ids: List, list of users

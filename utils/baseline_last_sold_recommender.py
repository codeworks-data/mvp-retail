...
class LastSoldRecommender:
    def fit(self, transactions_df: pd.DataFrame) -> None:
        """
        Set as attributes the last sold items for known customers, and overall last sold items for unknown customers
    ...
    def predict(self, users_ids: List) -> pd.DataFrame:
        """
        Return the ordered (by date) list of items sold for each user
